...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /admin/login (http://localhost:3030/admin/login) to access the admin login form.
        await page.goto("http://localhost:3030/admin/login")
        
        # -> Fill in the email and password fields with the admin credentials and submit the login form by clicking 'Sign In'.
        ...
        # -> Open the Site Settings page by clicking the 'Site Settings & SEO' link in Quick Actions.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[3]/div/a[4]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Open the 'Settings & SEO' page by clicking the sidebar 'Settings & SEO' link.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/aside/nav/a[9]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill a new value into the Logo Text field (index 7880) and save the Site Settings; then wait for the UI confirmation.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div/form/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('MamakGames — Admin Test')
